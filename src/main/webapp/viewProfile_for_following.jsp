...
<%Vector vgrd=jf.getValue("select college,percent,passing_yr,course from academics where course<>'HSC' and course<>'SSC' and userid='"+request.getParameter("userid").toString().trim()+"'", 4); %>
<%if(vgrd.size()==0){ %>
 <div class="jumbotron alert-success">No Record Found!!</div>
<br>
<%}else{ %>
<h6><p><%=vgrd.elementAt(3).toString().trim() %> from <%=vgrd.elementAt(0).toString().trim() %></p>
...
<%Vector vhsc=jf.getValue("select college,percent,passing_yr from academics where course='HSC' and userid='"+request.getParameter("userid").toString().trim()+"'", 3); %>
<%if(vhsc.size()==0){ %>
<div class="jumbotron alert-success">No Record Found!!</div>
<%}else{ %>
<h6><p>HSC from <%=vhsc.elementAt(0).toString().trim() %></p>
<i><P class="graduation-info">Scored Percentage, Passing Year </P> </i></h6>
...
<%Vector vssc=jf.getValue("select college,percent,passing_yr from academics where course='SSC' and userid='"+request.getParameter("userid").toString().trim()+"'", 3); %>
<%if(vssc.size()==0){ %>
<div class="jumbotron alert-success">No Record Found!!</div>
<%}else{ %>
<h6><p>SSC from <%=vssc.elementAt(0).toString().trim() %></p>
<i><P class="graduation-info">Scored Percentage, Passing Year </P> </i></h6>
  ...
  <%} %>
    ...
    <h4>Government Exam Categories you are interested in </h4>
    <p><%=v.elementAt(0).toString().trim() %></p>
        <h4>Exams you are preparing for</h4>
        <%
       Vector vskills=jf.getValue("select exam from stud_exams where userid='"+request.getParameter("userid").toString().trim()+"'", 1);
       ...
       <b><%=String.join(", ", vskills) %></b>
        ...
        <div id="examForm" style="display:none;">
            ...
            Vector v5=jf.getValue("select exam from stud_exams  where userid='"+request.getParameter("userid").toString().trim()+"'", 1);
            %>
