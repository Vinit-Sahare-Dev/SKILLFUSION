...
<div class="col-md-3">
	...
	<%} %>
	<a href="WorkingProfessional_Profile.jsp" class="button"><i class="fa fa-user-circle" ></i> Update Profile</a>
        </div>
    ...
    <div class="main-content">
        ...
        <div class="section">
            ...
            <div class="post-options">
                <button class="button" onclick="openPopup('post')">Post Answer</button>
                <br/>
               <h2><%=request.getParameter("ques").toString().trim() %></h2>
                <%JavaFuns jf1=new JavaFuns();
                Vector vp=jf1.getValue("select * from  forum_ans where qid="+request.getParameter("quesid").trim()+" order by pid desc", 9);
                
                %> </div><div class="post-options">
                <table class="table table-bordered">
                 
                <% for(int i=0;i<vp.size();i=i+9){ %>
                <tr>
                <td colspan="2"><h3> <%=vp.elementAt(i+5).toString() %> </h3></td>
               </tr><tr> <td colspan="2">Attachment:
               <%if(vp.elementAt(i+6).toString().equals("NA")){ %>
               NA |
               <%}else{ %>
               <a href="Answers/<%=vp.elementAt(i+6).toString() %>" target="_blank">Download</a> | 
               <%} %>
                Posted by <%=vp.elementAt(i+2).toString() %>(<%=vp.elementAt(i+3).toString() %>)
    ...
    <div id="popup-overlay" class="popup-overlay">
        ...
        <button class="close" onclick="closePopup('post')">Close</button>
            <h3>Submit Answer</h3>
            <form name="frm" method="post" action="RegAns" enctype="multipart/form-data">
            <input type="hidden" name="ques" value="<%=request.getParameter("ques").toString().trim() %>"/>
            <input type="hidden" name="qid" value="<%=request.getParameter("quesid").toString().trim() %>"/>
             <input type="file"  name="file" accept="*/*">
            <textarea name="ans" required placeholder="Write your Answer"></textarea>
